...
public class HomeWork2_ModelAnswers {
	...
	private static HashMap<String, Score> createInterviewsList(HashMap<String, Score> map) {
		...
		HashMap<String, Score> res = new HashMap<String, Score>();
		for (String name : map.keySet()) {
			Score score = new Score();
			score.setJapanese(map.get(name).getJapanese() - average.getJapanese() > 0 ? map.get(name).getJapanese() - average.getJapanese() : 0);
			score.setMath(map.get(name).getMath() - average.getMath() > 0 ? map.get(name).getMath() - average.getMath() : 0);
			score.setEnglish(map.get(name).getEnglish() - average.getEnglish() > 0 ? map.get(name).getEnglish() - average.getEnglish() : 0);
			res.put(name, score);
		}
